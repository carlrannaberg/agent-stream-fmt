...
function generateReport(): string {
  ...
  for (const vendor of vendors) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const sortedEventTypes = Array.from(analysis.eventTypes.entries())
          .sort((a, b) => b[1] - a[1]);
      ...
      report.push(`Error reading ${vendor} fixtures: ${(error as Error).message}\n`);
    }
  }
  ...
  const sortedAllEventTypes = Array.from(allEventTypes.entries())
    .sort((a, b) => b[1] - a[1]);
