...
async function main(): Promise<void> {
  console.log(
    `${colors.bold}${colors.cyan}🧪 Running Comprehensive Test Suite${colors.reset}`,
  );
  console.log(
    `${colors.dim}Testing Phase 1 components against all fixtures${colors.reset}\n`,
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(error => {
    console.error(
      `${colors.red}Error running test suite:${colors.reset}`,
      error,
    );
    process.exit(1);
  });
