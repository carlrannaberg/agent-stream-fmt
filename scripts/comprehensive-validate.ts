  ...
  for (const [index, line] of lines.entries()) {
    ...
    if (detected) {
      ...
      try {
        ...
        if (
          !filename.includes('error-handling') ||
...
function validateVendor(vendor: string): VendorResult {
  ...
  if (result.totalLines > 0) {
    result.averageLinesPerSecond =
      (result.totalLines / result.totalParseTime) * 1000;

    const totalDetections = result.files.reduce(
      (sum, file) => sum + file.parseResults.filter(r => r.detected).length,
      0,
    );
    const correctDetections = result.files.reduce(
      (sum, file) =>
        sum +
        file.parseResults.filter(r => r.detected && r.detectedVendor === vendor)
          .length,
      0,
    );

    result.detectionAccuracy =
      totalDetections > 0 ? (correctDetections / totalDetections) * 100 : 0;
  }
...
function runPerformanceBenchmarks(): {
  ...
  const testLine =
    '{"type":"message","role":"assistant","content":"Performance test message"}';
  ...
  const singleLinePerformance =
    (10000 / (singleEndTime - singleStartTime)) * 1000;
  ...
  const bulkProcessingPerformance =
    (testLines.length / (bulkEndTime - bulkStartTime)) * 1000;
...
function generateReport(results: VendorResult[]): ValidationReport {
  ...
  const report: ValidationReport = {
    ...
    performanceBenchmarks: {
      minLinesPerSecond: Math.min(
        ...performanceValues,
        benchmarks.singleLinePerformance,
      ),
      maxLinesPerSecond: Math.max(
        ...performanceValues,
        benchmarks.singleLinePerformance,
      ),
...
function printResults(report: ValidationReport): void {
  console.log(
    `${colors.bold}${colors.cyan}📋 Comprehensive Validation Report${colors.reset}`,
  );
  ...
  console.log(
    `  Parsers: ${report.overallStats.availableParsers.join(', ')}\n`,
  );
  ...
  console.log(
    `  Average performance: ${report.overallStats.averagePerformance.toFixed(2)} lines/sec\n`,
  );

  // Performance benchmarks
  console.log(`${colors.bold}Performance Benchmarks:${colors.reset}`);
  console.log(
    `  Target: ${report.performanceBenchmarks.performanceTarget} lines/sec`,
  );
  console.log(
    `  Average: ${report.performanceBenchmarks.averageLinesPerSecond.toFixed(2)} lines/sec`,
  );
  console.log(
    `  Range: ${report.performanceBenchmarks.minLinesPerSecond.toFixed(2)} - ${report.performanceBenchmarks.maxLinesPerSecond.toFixed(2)} lines/sec`,
  );
  ...
  for (const vendor of report.vendors) {
    ...
    console.log(
      `  Performance: ${vendor.averageLinesPerSecond.toFixed(2)} lines/sec`,
    );
    console.log(
      `  Detection accuracy: ${vendor.detectionAccuracy.toFixed(1)}%`,
    );
    ...
    for (const file of vendor.files) {
      ...
      const performanceColor =
        file.linesPerSecond >= 1000 ? colors.green : colors.yellow;

      console.log(`    ${fileStatus} ${file.filename}`);
      console.log(`      Lines: ${file.totalLines} (${file.validLines} valid)`);
      console.log(
        `      Performance: ${performanceColor}${file.linesPerSecond.toFixed(2)} lines/sec${colors.reset}`,
      );
      console.log(
        `      Parse time: ${formatDuration(file.averageParseTime)} avg`,
      );
      ...
      if (file.errors.length > 0) {
        ...
        const errorsByType = file.errors.reduce(
          (acc, error) => {
  ...
  // Final status
  const overallStatus =
    report.overallStats.totalErrors === 0 &&
    report.performanceBenchmarks.meetsTarget
...
function main(): void {
  ...
  console.log(
    `${colors.bold}${colors.cyan}🔍 Running Comprehensive Validation...${colors.reset}\n`,
  );
