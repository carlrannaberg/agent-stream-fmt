...
describe('Stream events', () => {
  ...
  describe('error handling', () => {
    it('continues on parse errors by default', async () => {
      ...
      expect(errorEvents[0].message.toLowerCase()).toContain('invalid json');
    });
  ...
  describe('debug events', () => {
    it('emits debug events when enabled', async () => {
      ...
      const detectionEvent = debugEvents.find(e => e.raw?.detected);
      if (detectionEvent) {
        expect(detectionEvent.raw.detected).toBeDefined();
      }
