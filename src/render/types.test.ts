...
describe('Render Types', () => {
  ...
  it('should enforce format enum values', () => {
    ...
    const invalidOpts: RenderOptions = { format: 'xml' };
  });
  ...
  it('should type check RendererRegistry', () => {
    const registry: RendererRegistry = {
      ansi: (options) => ({
        render: (event) => `ANSI: ${event.t}`,
        renderBatch: (events) => events.map(e => `ANSI: ${e.t}`).join('\n'),
        flush: () => ''
      }),
      html: (options) => ({
        render: (event) => `<div>${event.t}</div>`,
        renderBatch: (events) => events.map(e => `<div>${e.t}</div>`).join(''),
        flush: () => '</body></html>'
      }),
      json: (options) => ({
        render: (event) => JSON.stringify(event),
        renderBatch: (events) => JSON.stringify(events),
        flush: () => ''
      })
    };
