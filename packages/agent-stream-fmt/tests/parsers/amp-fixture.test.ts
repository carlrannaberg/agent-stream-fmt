...
describe('AmpParser with real fixtures', () => {
  ...
  describe('build-process.jsonl', () => {
    it('should parse complex build process', () => {
      ...
      // Count event types
      const startEvents = events.filter(e => e.t === 'tool' && e.phase === 'start');
      const outputEvents = events.filter(e => e.t === 'tool' && (e.phase === 'stdout' || e.phase === 'stderr'));
  ...
  describe('test-execution.jsonl', () => {
    it('should parse test execution with mixed output types', () => {
      ...
      // Find stderr events
      const stderrEvents = events.filter(e => e.t === 'tool' && e.phase === 'stderr');
      const stdoutEvents = events.filter(e => e.t === 'tool' && e.phase === 'stdout');
  ...
  describe('all fixtures', () => {
    it('should detect all fixture lines as Amp format', () => {
      const fixtures = ['simple-task.jsonl', 'build-process.jsonl', 'test-execution.jsonl'];
