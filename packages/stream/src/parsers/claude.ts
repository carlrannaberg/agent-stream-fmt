...
import { AgentEvent, MessageEvent, ToolEvent, CostEvent, ErrorEvent } from '../types.js';
...
export class ClaudeParser implements VendorParser {
  ...
  private parseToolResult(obj: Record<string, unknown>): ToolEvent[] {
    ...
    const toolName = typeof obj.tool_use_id === 'string' ? obj.tool_use_id : 'unknown';
    ...
    if (obj.error) {
      events.push({
        ...
        text: typeof obj.error === 'string' ? obj.error : 
              (typeof obj.error === 'object' && obj.error && 'message' in obj.error && typeof obj.error.message === 'string') ? obj.error.message : JSON.stringify(obj.error)
      });
    }
