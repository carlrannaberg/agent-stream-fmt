...
import { AgentEvent, MessageEvent, CostEvent } from '../types.js';
...
export class GeminiParser implements VendorParser {
  ...
  parse(line: string): AgentEvent[] {
    ...
    switch (record.type) {
      case 'user':
        events.push(this.parseMessage(record, 'user'));
        break;
        
      case 'assistant':
        events.push(this.parseMessage(record, 'assistant'));
        break;
        
      case 'metadata': {
        const cost = this.parseMetadata(record);
   ...
   */
  private parseMessage(obj: Record<string, unknown>, role: 'user' | 'assistant'): MessageEvent {
    return {
      t: 'msg',
      role,
   ...
   */
  private parseMetadata(obj: Record<string, unknown>): CostEvent | null {
    if (!obj.usage || typeof obj.usage !== 'object' || obj.usage === null) return null;
