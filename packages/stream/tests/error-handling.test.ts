...
describe('Comprehensive Error Handling Tests', () => {
  ...
  describe('JSON Parsing Errors', () => {
    ...
    it('handles malformed JSON gracefully in detection', () => {
      for (const [index, invalidJson] of invalidJsonCases.entries()) {
        ...
        expect(detected, `Invalid JSON case ${index}: ${invalidJson}`).toBeNull();
      }
    });
    ...
    it('throws ParseError for malformed JSON in parsing', () => {
      ...
      for (const [index, invalidJson] of invalidJsonCases.entries()) {
        expect(() => parser.parse(invalidJson), 
    ...
    it('provides meaningful error messages for malformed JSON', () => {
      ...
      const testCases = [
        { input: '{"type":"message","content":"unclosed', expectedPattern: /Invalid JSON/ },
  ...
  describe('Edge Case Inputs', () => {
    it('handles empty and whitespace-only inputs', () => {
      ...
      for (const input of edgeCases) {
        ...
        expect(detected, `Empty/whitespace input should return null: ${JSON.stringify(input)}`).toBeNull();
    ...
    it('handles unicode and special characters', () => {
      ...
      for (const content of unicodeTestCases) {
        ...
        expect(detected?.vendor, `Unicode test case should be detected: ${content}`).toBe('claude');
        
        const events = parser.parse(message);
        expect(events).toHaveLength(1);
        expect(events[0].text, `Unicode content should be preserved: ${content}`).toBe(content);
      }
    });
    ...
    it('handles special whitespace and line endings', () => {
      ...
      for (const content of whitespaceTestCases) {
        ...
        const detected = detectVendor(message);
        expect(detected?.vendor, `Whitespace test case should be detected: ${JSON.stringify(content)}`).toBe('claude');
        
        const events = parser.parse(message);
        expect(events).toHaveLength(1);
        expect(events[0].text, `Whitespace should be preserved: ${JSON.stringify(content)}`).toBe(content);
      }
    });
  });
  ...
  describe('Unknown Event Types', () => {
    it('handles unknown event types gracefully', () => {
      ...
      for (const eventType of unknownEventTypes) {
        ...
        expect(detected, `Unknown event type should not be detected: ${eventType}`).toBeNull();
  ...
  describe('Parser Registration Errors', () => {
    it('handles invalid parser registrations', () => {
      const testCases = [
        ...
        { parser: { vendor: '', detect: () => true, parse: () => [] }, error: 'Parser must have a valid vendor name' },
        { parser: { vendor: '   ', detect: () => true, parse: () => [] }, error: 'Parser must have a valid vendor name' },
        { parser: { vendor: 'auto', detect: () => true, parse: () => [] }, error: "Cannot register parser with vendor 'auto'" },
      ];
      
      for (const testCase of testCases) {
        expect(() => {
          registry.registerParser(testCase.parser as any);
        }, `Should throw error for invalid parser: ${JSON.stringify(testCase.parser)}`).toThrow(testCase.error);
      }
    });
    ...
    it('handles invalid priority values', () => {
      ...
      for (const priority of invalidPriorities) {
        ...
        }, `Should throw error for invalid priority: ${priority}`).toThrow('Priority must be a finite number');
      }
    });
  });
  ...
  describe('Detection Failures', () => {
    it('handles detection failures gracefully', () => {
      ...
      for (const input of nonMatchingInputs) {
        const detected = detectVendor(input);
        expect(detected, `Non-matching input should return null: ${input}`).toBeNull();
        
        // selectParser with auto should throw for undetected formats
        expect(() => selectParser('auto', input), 
    ...
    it('handles selectParser errors correctly', () => {
      ...
      expect(() => selectParser('auto', '{"unknown":"format"}')).toThrow('Failed to auto-detect vendor from line');
    });
  });
  ...
  describe('Malformed JSON Validation', () => {
    it('properly validates malformed JSON in error-handling fixtures', () => {
      ...
      expect(validJsonCount, 'Should have some valid JSON lines').toBeGreaterThan(0);
      expect(invalidJsonCount, 'Should have some invalid JSON lines').toBeGreaterThan(0);
      expect(parseErrorCount, 'Should have parse errors for invalid JSON').toBe(invalidJsonCount);
    });
    ...
    it('processes error-handling fixtures correctly', () => {
      ...
      for (const [lineIndex, line] of lines.entries()) {
        try {
          ...
          expect(detected?.vendor, `Line ${lineIndex + 1} should be detected as Claude`).toBe('claude');
          ...
          const detected = detectVendor(line);
          expect(detected, `Line ${lineIndex + 1} with invalid JSON should not be detected`).toBeNull();
          
          // Parser should throw ParseError for invalid JSON
          try {
            parser.parse(line);
            expect.fail(`Line ${lineIndex + 1} should throw ParseError for invalid JSON`);
          } catch (parseError) {
            parseErrors++;
      ...
      expect(invalidLines, 'Error handling fixture should contain some invalid JSON').toBeGreaterThan(0);
      expect(parseErrors, 'Should have parse errors for invalid JSON').toBe(invalidLines);
    });
  });
  
  describe('Memory and Performance Under Error Conditions', () => {
    it('handles repeated parsing failures efficiently', () => {
      const parser = selectParser('claude');
      const invalidJson = '{"type":"message","content":"unclosed';
    ...
    it('handles detection failures efficiently', () => {
      ...
      // Should handle detection failures efficiently
      expect(nullResults).toBe(10000);
      expect(endTime - startTime).toBeLessThan(500); // Should complete in <0.5 second
    });
  });
});
