...
import { isAgentEvent, isMessageEvent, isToolEvent, isCostEvent, isErrorEvent } from './guards.js';

describe('Type guards', () => {
  describe('isAgentEvent', () => {
    it('should return true for valid agent events', () => {
      expect(isAgentEvent({ t: 'msg', role: 'user', text: 'Hello' })).toBe(true);
      expect(isAgentEvent({ t: 'tool', name: 'bash', phase: 'start' })).toBe(true);
  ...
  describe('isMessageEvent', () => {
    it('should validate message events correctly', () => {
      expect(isMessageEvent({ t: 'msg', role: 'user', text: 'Hello' })).toBe(true);
      expect(isMessageEvent({ t: 'msg', role: 'assistant', text: '' })).toBe(true);
      expect(isMessageEvent({ t: 'msg', role: 'system', text: 'System message' })).toBe(true);
    });

    it('should reject invalid message events', () => {
      expect(isMessageEvent({ t: 'msg' })).toBe(false);
      expect(isMessageEvent({ t: 'msg', role: 'invalid', text: 'Hello' })).toBe(false);
      expect(isMessageEvent({ t: 'msg', role: 'user' })).toBe(false);
      expect(isMessageEvent({ t: 'tool', name: 'bash', phase: 'start' })).toBe(false);
    });
  });

  describe('isToolEvent', () => {
    it('should validate tool events correctly', () => {
      expect(isToolEvent({ t: 'tool', name: 'bash', phase: 'start' })).toBe(true);
      expect(isToolEvent({ t: 'tool', name: 'python', phase: 'stdout', text: 'output' })).toBe(true);
      expect(isToolEvent({ t: 'tool', name: 'node', phase: 'end', exitCode: 0 })).toBe(true);
    });
    ...
    it('should reject invalid tool events', () => {
      ...
      expect(isToolEvent({ t: 'msg', role: 'user', text: 'Hello' })).toBe(false);
    });
  });
  ...
  describe('isCostEvent', () => {
    ...
    it('should reject invalid cost events', () => {
      ...
      expect(isCostEvent({ t: 'msg', role: 'user', text: 'Hello' })).toBe(false);
    });
  });

  describe('isErrorEvent', () => {
    it('should validate error events correctly', () => {
      expect(isErrorEvent({ t: 'error', message: 'Something went wrong' })).toBe(true);
    ...
    it('should reject invalid error events', () => {
      ...
      expect(isErrorEvent({ t: 'msg', role: 'user', text: 'Hello' })).toBe(false);
    });
  });
});