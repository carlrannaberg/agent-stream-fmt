...
describe('Fixture Validation Tests', () => {
  ...
  beforeAll(() => {
    ...
    for (const vendor of vendors) {
      ...
      for (const filename of files) {
        ...
        const invalidLines: { line: string; lineNumber: number; error: string }[] = [];
    ...
    it('all fixture files contain valid JSON lines', () => {
      ...
      for (const fixture of allFixtures) {
        ...
        if (fixture.filename.includes('error-handling')) {
          expect(fixture.invalidLines.length, 
          ...
          const invalidRatio = fixture.invalidLines.length / fixture.lines.length;
          expect(invalidRatio, 
      ...
      console.log(`  Success rate: ${((validLines / totalLines) * 100).toFixed(1)}%`);
  ...
  describe('Vendor Detection Validation', () => {
    it('detects correct vendor for all valid fixture lines', () => {
        ...
        for (const line of fixture.validLines) {
          ...
          if (detected) {
            ...
            if (detected.vendor === fixture.vendor) {
              ...
              console.warn(`Wrong vendor detected for ${fixture.vendor}/${fixture.filename}: expected ${fixture.vendor}, got ${detected.vendor}`);
            }
          } else {
            ...
            if (fixture.vendor === 'claude') {
              console.warn(`Undetected Claude line in ${fixture.filename}: ${line}`);
            }
          }
        }
      }
      ...
      console.log(`  Detection rate: ${((detectionStats.detected / detectionStats.total) * 100).toFixed(1)}%`);
      console.log(`  Accuracy rate: ${((detectionStats.correctVendor / detectionStats.detected) * 100).toFixed(1)}%`);
      
      // For Claude fixtures, expect high detection rate
      const claudeFixtures = allFixtures.filter(f => f.vendor === 'claude');
      const claudeLines = claudeFixtures.reduce((sum, f) => sum + f.validLines.length, 0);
      const claudeDetected = claudeFixtures.reduce((sum, fixture) => {
        return sum + fixture.validLines.filter(line => {
          const detected = detectVendor(line);
  ...
  describe('Parser Processing Validation', () => {
    it('successfully processes all detectable fixture lines', () => {
          ...
          if (detected) {
            ...
            try {
              ...
              for (const event of events) {
                ...
                expect(['msg', 'tool', 'cost', 'error', 'debug'], 
              ...
              if (!fixture.filename.includes('error-handling')) {
                throw new Error(`Failed to process line in ${fixture.vendor}/${fixture.filename}: ${line}\nError: ${error}`);
              }
            }
          }
        }
      ...
      console.log(`  Success rate: ${((processingStats.processed / processingStats.total) * 100).toFixed(1)}%`);
      console.log(`  Average events per line: ${(processingStats.totalEvents / processingStats.processed).toFixed(2)}`);
      
      expect(processingStats.processed, 'Should process some lines').toBeGreaterThan(0);
      expect(processingStats.totalEvents, 'Should generate some events').toBeGreaterThan(0);
    });
    ...
    it('processes different event types correctly', () => {
      ...
      expect(Object.keys(eventTypeStats).length, 'Should have multiple event types').toBeGreaterThan(1);
    });
  });
  ...
  describe('Coverage Validation', () => {
    it('provides adequate test coverage across all fixtures', () => {
      ...
      console.log(`  Vendors with fixtures: ${coverageStats.vendorsWithFixtures}`);
      ...
      const processingCoverage = (coverageStats.processedLines / coverageStats.validLines) * 100;
      console.log(`  Processing coverage: ${processingCoverage.toFixed(1)}%`);
      
      // Should have good coverage
      expect(coverageStats.vendorsWithFixtures, 'Should have fixtures for multiple vendors').toBeGreaterThan(0);
      expect(coverageStats.totalFiles, 'Should have multiple fixture files').toBeGreaterThan(0);
      expect(coverageStats.totalLines, 'Should have substantial test data').toBeGreaterThan(10);
      expect(processingCoverage, 'Should process most valid lines').toBeGreaterThan(50);
    });
  });
    ...
    it('provides comprehensive fixture overview', () => {
      ...
      for (const vendor of vendors) {
        ...
        for (const fixture of vendorFixtures) {
          ...
          console.log(`    Lines: ${fixture.lines.length} (${fixture.validLines.length} valid, ${fixture.invalidLines.length} invalid)`);
        }
        
        console.log(`  Total: ${vendorTotal.files} files, ${vendorTotal.lines} lines, ${vendorTotal.processedLines} processed, ${vendorTotal.events} events`);
      ...
      console.log(`  Processing rate: ${((grandTotal.processedLines / grandTotal.validLines) * 100).toFixed(1)}%`);
      console.log(`${Array(60).fill('=').join('')}`);
